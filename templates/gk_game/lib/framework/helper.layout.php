...
class GKTemplateLayout {
	...
	public function generateLayoutWidths() {
		...
		if($this->API->modules('sidebar')) {
               ...
               if($this->API->get('sidebar_position', 'right') == 'left') {
                       $this->API->addCSSRule('#gkPageContent { background-position: '.$sidebar_width.'% 0!important; }');
               } else {
                       $this->API->addCSSRule('#gkPageContent { background-position: '.$content_width.'% 0!important; }');
               }
       } else {
               $this->API->addCSSRule('#gkPageContent { background-image: none!important; }');
       }
       //
       if($this->API->modules('inset')) {
               ...
               if($this->API->get('inset_position', 'right') == 'left') {
                       $this->API->addCSSRule('#gkContent { background-position: '.$inset_width.'% 0!important; }');
               } else {
                       $this->API->addCSSRule('#gkContent { background-position: '.$contentwrap_width.'% 0!important; }');
               }
       } else {
       ...
       if(
       	$this->API->modules('article_inset') &&
       	(
       		($option_val == 'com_content' || $option_val == 'com_k2') &&
       		($view_val == 'item' || $view_val == 'article')
       	)	
       ) {
	       	$articlewrap_width = 100 - $inset_width;
	       	// generate inset width
	       	$this->API->addCSSRule('article .itemBody { width: '.$articlewrap_width.'%; }' . "\n");
	       	$this->API->addCSSRule('#gkArticleInset { width: '.$inset_width.'%; }' . "\n");
       }
       //
